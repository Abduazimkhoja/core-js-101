...
function getRectangleString(width, height) {
  ...
  const middleLines = middle.repeat(height - 2);

  return top + middleLines + bottom;
}
...
function encodeToRot13(str) {
  return str
    ...
    .map((item) => {
      ...
      const isLetter = (itemAscii > 64 && itemAscii < 91)
        || (itemAscii > 96 && itemAscii < 123);

      if (!isLetter) return item;

      const itemAsciiDiapason = (itemAscii <= 90 && itemAscii + 13 > 90)
